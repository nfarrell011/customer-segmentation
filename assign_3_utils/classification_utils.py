...
def class_thresh_metrics_class_perf_assess_binary(best_model, estimator_names, grid_search_cv_results_df,
...
def precision_recall_bootstrap_no_refit_binary(estimator_names, grid_search_cv_results_df, cap_x_df, y_df,
                                               n_bootstrap=10, prc_pos_label=None, prc_sample_weight=None,
                                               prc_drop_intermediate=False, data_set_name=None,
...
def bootstrap_statistics_helper(sample_list, sample_name, statistic=np.mean, conf_level=0.95):
...
def roc_curve_bootstrap_no_refit_binary(estimator_names, grid_search_cv_results_df, cap_x_df, y_df, n_bootstrap=10,
                                        roc_pos_label=None, roc_sample_weight=None, roc_drop_intermediate=True,
                                        roc_score_average='macro', roc_score_sample_weight=None, roc_score_max_fpr=None,
    ...
    for estimator_name in estimator_names:
        ...
        temp_results_df = \
            roc_curve_bootstrap_no_refit_helper(estimator, cap_x_df, y_df, roc_kwargs, roc_score_kwargs, n_bootstrap,
                                                data_set_name, model_selection_stage, classification_threshold,
                                                estimator_name)
...
def roc_curve_bootstrap_no_refit_helper(trained_classifier, cap_x_df, y_df, roc_kwargs, roc_score_kwargs, n_bootstrap,
    ...
    bootstrap_statistics_helper(class_thresh_tpr_list, 'trp')
    bootstrap_statistics_helper(class_thresh_fpr_list, 'fpr')

    return pd.DataFrame(df_row_dict_list)


def plot_errors_as_a_function_of_classification_threshold(best_model, estimator_names, grid_search_cv_results_df,
